 ...
 *
 * Renders the header (title, theme swatches, search, sort, actions) and
 * the responsive project card grid. Reacts live to package settings and
 * store changes.
 */
 ...
 * Newer Electron builds removed `File.path`, so fall back to the
 * `webUtils.getPathForFile` helper when available.
 *
 * @param {File} file dropped file entry
 * @returns {string|null} absolute path, or null when unavailable
 */
...
class HubView {
  /**
   * @param {ProjectStore} projectStore shared project store instance
   * @param {Object} [options]
   * @param {string} [options.uri] pane item URI
   */
  constructor(projectStore, { uri } = {}) {
    ...
    // Root element. Data attributes and CSS variables below mirror the
    // package settings so the stylesheet can react without re-rendering.
    ...
    // Decorative background blobs (visible behind the glass panels).
   ...
   * Accepts both Color objects (from the color setting) and raw strings.
   */
  applyAccent(value) {
  ...
  /**
   * Returns the filtered + sorted project list according to settings
   * and the current search box content.
  ...
  render() {
    ...
    // Debounce the search box so typing stays smooth on big lists.
  ...
  /**
   * Opens a project, either in a new window or into the current one,
   * and records the "last opened" metadata.
   */
  openProject(project) {
